...
const char* my_inet_ntop(int af, const void* src, char* dst, socklen_t size)
{
    ...
    for (int i = 0; i < 4; ++i)
	{
		...
		else
		{
            while (byte > 0)
			{
                byte_str = char('0' + (byte % 10)) + byte_str;
                byte /= 10;
            }
        }
        result += byte_str;
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                else if (cgi_fd_to_client_fd.count(fd))
                {
                    ...
                    if (ev & (EPOLLERR | EPOLLHUP))
                    {
                        ...
                        std::cerr << "CGI fd " << fd << " error or hangup" << std::endl;
